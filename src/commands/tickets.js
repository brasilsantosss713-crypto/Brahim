...
async function postTicketPanel(guildId, channel, title, intro, categories) {
  ...
  store.setSettings(guildId, settings);
...
module.exports = [
...

  {
    ...
    async execute(interaction) {
      ...
      await postTicketPanel(interaction.guild.id, channel, title, intro, categories);
...

  {
    data: new SlashCommandBuilder()
      ...
      .setDescription('Post your pre-configured ticket panel to #🎫»┃open-ticket (no typing needed)')
      .addChannelOption(o => o.setName('channel').setDescription('Override: pick a different channel than the default').addChannelTypes(ChannelType.GuildText))
      .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild),
    async execute(interaction) {
      const channel = interaction.options.getChannel('channel')
        || interaction.guild.channels.cache.find(c => c.name === '🎫»┃open-ticket');

      if (!channel) {
        return interaction.reply({
          content: "Couldn't find a channel named `🎫»┃open-ticket`. Either run `/setup` first to create it, or pass a `channel` option to this command to pick one manually.",
          ephemeral: true,
        });
      }

      const categories = [
        { name: 'Giveaway Claim', desc: 'Claim a Giveaway always provide proof.' },
        { name: 'Partnership', desc: 'Partnering' },
        { name: 'Market', desc: 'Buy or sell skellies.' },
        { name: 'Giveaway Sponsors', desc: 'Sponsor a giveaway' },
        { name: 'Build Requests', desc: 'Request a build make sure too send a schematic.' },
        { name: 'General Support', desc: 'Open a ticket for support or any questions!' },
      ];

      await postTicketPanel(interaction.guild.id, channel, 'Tickets', 'Open a ticket below', categories);
